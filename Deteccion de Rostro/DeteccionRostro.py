...
while(capturador_video.isOpened()):
  ...
  dibujar_cuadro_rostro(rostros_detectados, frame, color_cuadro, grosor_linea) #Dibuja en el face un cuadro con el color deseado

  if is_exito == True:
    cv2.imshow('WebCam Rostros Detected', frame)

    if cv2.waitKey(1) == 27: #Cuando es letra usar cv2.waitKey(25) & 0xFF == ord('q'):, para usar mayusucula o minuscula juntos
      break
  else:
    break
